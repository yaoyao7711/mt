...
visualizer.logger_structure(model.netG)
visualizer.logger_structure(model.netD)
total_steps = 0
...
for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
    ...
    if opt.display_id > 0:
        ...
        iter_start_time = time.time()
        ssim, mssim, l1, psnr, lpips = eval.eval(model, visualizer)
        t = (time.time() - iter_start_time) / opt.batchSize
        metrix = eval.get_eval_metrix(ssim, mssim, l1, psnr, lpips)
        visualizer.print_current_errors(epoch, epoch_iter, metrix, t, t)

    # if epoch % opt.save_epoch_freq == 0:
    if ssim > val_ssim and l1<val_l1:
        val_ssim = ssim
        val_l1 = l1
        print('saving the model at the end of epoch %d, iters %d' % (epoch, total_steps))
        model.save(f'best_{epoch}')
        hist = {'plot_data': visualizer.plot_data, 'metric': visualizer.metric_data}
        p = os.path.join(model.save_dir, "history.pth")
        torch.save(hist, p)
        # model.save(epoch)
